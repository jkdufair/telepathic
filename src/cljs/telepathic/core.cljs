...
(defn render-actions
  ...
  [:div {:id "action-cards"
   ...
   (map-indexed (fn [index action]
                  [:img {:src (str "img/" (name action) ".png")
                         :class (str (cond
...
(defn render-conditions
  "Render the condition img for the current player"
  []
